...
ASelfDrivingCarHud::ASelfDrivingCarHud()
{
	static ConstructorHelpers::FObjectFinder<UFont> Font(TEXT("/Engine/EngineFonts/RobotoDistanceField"));
	HUDFont = Font.Object;
}

void ASelfDrivingCarHud::DrawHUD()
{
	Super::DrawHUD();
	...
	if( bHMDDeviceActive == false )
	{
		...
		ASelfDrivingCarPawn* Vehicle = Cast<ASelfDrivingCarPawn>(GetOwningPawn());
		if ((Vehicle != nullptr) && (Vehicle->bInCarCameraActive == false))
		{
			...
			// Speed Error
			FCanvasTextItem SpeedErrorTextItem(FVector2D(HUDXRatio * 805.f, HUDYRatio * 555), Vehicle->SpeedErrorDisplayString, HUDFont, FLinearColor::Red);
			SpeedErrorTextItem.Scale = ScaleVec;
			Canvas->DrawItem(SpeedErrorTextItem);
			
			// Throttle Scale
			FCanvasTextItem ThrottleScaleItem(FVector2D(HUDXRatio * 805.f, HUDYRatio * 600), Vehicle->ThrottleDisplayString, HUDFont, FLinearColor::Red);
			ThrottleScaleItem.Scale = ScaleVec;
			Canvas->DrawItem(ThrottleScaleItem);
